...
§GRUND-product-purpose. The Gradle and Maven plugins must not introduce
...
pass-through: §gradle/FS-native-tasks.4 for Gradle build args and
§maven/FS-config-model.1 for Maven `<buildArgs>`. Mirroring every
...
(§gradle/FS-native-tasks.4 `quickBuild`, Maven `<quickBuild>`) is the canonical example: it is
...
§gradle/FS-plugin-model.4 — would otherwise force users into excessive
