...
This non-goal bounds §GOAL-build-tool-native-image-workflows under the grounding in
...
pass-through: §GRADLE-plugin.2.4 for Gradle build args and
§MAVEN-plugin.3.1 for Maven `<buildArgs>`. Mirroring every
...
§GRADLE-plugin.1.4 — would otherwise force users into excessive
