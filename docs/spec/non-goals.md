...
pass-through: §gradle/FS-gradle-native-image-tasks.4 for Gradle build args and
§maven/FS-maven-configuration-model.1 for Maven `<buildArgs>`. Mirroring every
...
(§gradle/FS-gradle-native-image-tasks.4 `quickBuild`, Maven `<quickBuild>`) is the canonical example: it is
...
§gradle/FS-gradle-plugin-model.4 — would otherwise force users into excessive
