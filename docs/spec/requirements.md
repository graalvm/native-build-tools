...
surface in §GRADLE-plugin.1.2 and the Maven goal and configuration
surface in §MAVEN-plugin.1 must stay compatible across minor and patch
releases.
...
users toward the current name, as required by §GRADLE-plugin.2.3 and
§MAVEN-plugin.1.1. Removal happens only on a major version boundary
