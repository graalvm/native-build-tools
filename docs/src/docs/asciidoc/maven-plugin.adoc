...
[[maven-list-missing-metadata-libs]]
...
- Writes a JSON report to _target/native/list-missing-metadata-libs.json_

To create issues automatically instead of printing links, opt in explicitly:

[source,bash, role="multi-language-sample"]
----
./mvnw -DcreateIssues=true -DgithubToken=... native:list-missing-metadata-libs
----
