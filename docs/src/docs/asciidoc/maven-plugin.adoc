...
* `standard` - in this mode you run agent only with options provided in `commonOptions` section.
* `direct` - in this mode you can provide command line that you want to be executed as an agent configuration. In this
     mode, user is fully responsible for agent configuration, and the rest of the config file will be ignored.
* `conditional` - in this mode you can provide additional files that can be used as a filter for the agent output (https://github.com/oracle/graalvm-reachability-metadata/blob/master/docs/CollectingMetadata.md[see this]). You
can read more about conditional mode https://www.graalvm.org/22.1/reference-manual/native-image/ExperimentalAgentOptions/#generating-conditional-configuration-using-the-agent[here].

Each option and how you should use it is described bellow:

* `enabled` - a simple flag that specifies whether the agent is on or off. Can be set to true or false.
* `defaultMode` - agent mode switcher. Can be set to: standard, direct or conditional.
* `modes` - list of additional mode options, specific for the mode type. Inside this tag, you can specify options for
 direct or conditional modes. Standard mode doesn't have any specific options.
* `commonOptions` - list of options that can be specify independent from agent mode. More about common options can be found
...
* `metadataCopy` - provides additional options for manipulating the agent output after agent finishes its job.
    ** You can copy agent output to the specific location (inside `<outputDirectory>`)
    ** Merge agent output with files located there (by specifying true or false inside `<merge>` tag)
    ** Disable some of the agent stages (by specifying the stage inside `<disabledStages>`)
