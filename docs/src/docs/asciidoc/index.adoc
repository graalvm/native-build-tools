...
Run `listMissingMetadataLibs` before troubleshooting metadata manually so you can see which direct dependencies are unsupported by the reachability metadata repository.
====
