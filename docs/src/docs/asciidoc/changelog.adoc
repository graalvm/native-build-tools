...
- Maven native test goals now read the matching Surefire or Failsafe execution, including its configured `testClassesDirectory`, and keep their test-ID output separate. §maven/FS-native-tests.6
...
- Breaking change: move to the reachability-metadata.json metadata format
* The plugins no longer use the global `metadata/index.json` and instead rely on the newer reachability metadata repository layout
* Added reachability metadata schema cross-validation
* Users may need to update dependencies or wait for dependent projects and metadata to adapt to the new metadata format
