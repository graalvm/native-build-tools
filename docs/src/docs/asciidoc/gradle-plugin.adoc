...
1. <<disabling-toolchains,Disabling toolchain support>>
2. Run Gradle itself with a GraalVM SDK
3. Setup a `GRAALVM_HOME` environment variable pointing to your GraalVM installation
...
This is the case if, for example, you want to use GraalVM Enterprise or that you want to be able to select a particular version of GraalVM.
...
Therefore, it becomes your responsibility to make sure that the JDK pointing at this environment variable matches your build script requirements (in particular, the language version).
