...
Agent modes are configured from DSL in the following manner:

.Configuring agent modes
[source, groovy, role="multi-language-sample"]
----
include::../snippets/gradle/groovy/build.gradle[tags=agent-modes]
----

[source, kotlin, role="multi-language-sample"]
----
include::../snippets/gradle/kotlin/build.gradle.kts[tags=agent-modes]
----

Consult link:javadocs/native-gradle-plugin/org/graalvm/buildtools/gradle/dsl/agent/AgentOptions.html[AgentOptions javadoc] for more configurable options.
...
You can also configure agent options per individual binary:

.Configuring agent options per binary
[source, groovy, role="multi-language-sample"]
----
