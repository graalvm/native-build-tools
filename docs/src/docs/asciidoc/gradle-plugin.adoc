...
[[gradle-list-missing-metadata-libs]]
...
- Writes a JSON report to _build/reports/native/list-missing-metadata-libs.json_

To create issues automatically instead of printing links, opt in explicitly:

[source,bash, role="multi-language-sample"]
----
./gradlew listMissingMetadataLibs -PcreateIssues=true
----
