...
After updating the filters, you can regenerate the metadata, which will result in the following _reachability-metadata.json_ file:
...
The plugin automatically passes arguments to the `native-image` tool using an argument file, which should prevent all https://github.com/graalvm/native-build-tools/issues/85[long classpath issues] under Windows.
...
For example, copying the reachability metadata into the application JAR can be useful when some other process is responsible for converting your JAR into a native image.
You might be generating a shaded JAR file and using a https://paketo.io/[Paketo buildpack] to convert it to a native image.
