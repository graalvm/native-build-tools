...
[source,bash, role="multi-language-sample"]
----
./gradlew listMissingMetadataLibs
----

The task prints missing direct runtime dependencies, reuses existing reachability-metadata issues when it can, and writes a JSON report to _build/reports/native/list-missing-metadata-libs.json_.
If you want the task to create GitHub issues automatically instead of printing links, run `./gradlew listMissingMetadataLibs -PcreateIssues=true`.
