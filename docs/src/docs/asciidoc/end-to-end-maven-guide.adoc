...
[source,bash, role="multi-language-sample"]
----
./mvnw native:list-missing-metadata-libs
----

The goal prints missing direct runtime dependencies, reuses existing reachability-metadata issues when it can, and writes a JSON report to _target/native/list-missing-metadata-libs.json_.
If you want the goal to create GitHub issues automatically instead of printing links, run `./mvnw -DcreateIssues=true native:list-missing-metadata-libs`.
